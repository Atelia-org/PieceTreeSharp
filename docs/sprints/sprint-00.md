...
- **Success Criteria:**
  ...
  - `src/PieceTree.TextBuffer/Core` 包含可编译的 RB Tree skeleton（PieceTreeNode、PieceTreeModel、balancing helpers）并通过 smoke `dotnet test`。
...
| P1 | PT-004 | 将 PieceTree RB Tree skeleton（节点结构、旋转/平衡、基础查询 API）迁移到 `src/PieceTree.TextBuffer/Core`，附带 stub search hook 与 smoke `dotnet test`。 | 2 | Porter-CS | 2025-11-22 | PT-003 通过审阅 | ⏳ 2025-11-19：G1 落地 PieceTreeBuilder + `PieceTreeBuffer` 接线并通过 `dotnet test`；G2 需实现增量编辑/EnumeratePieces/Search stub TODO。 |
| P2 | PT-005 | 建立 QA 测试矩阵与基准计划：更新 `PieceTree.TextBuffer.Tests/UnitTest1.cs`、记录首个 `dotnet test` 输出并在矩阵中登记覆盖面。 | 1 | QA-Automation | 2025-11-23 | PT-004 代码 drop | ✅ 2025-11-19：创建 `TestMatrix.md`、扩展 7 个 Fact 覆盖 Plain/CRLF/Multi-chunk、记录基线测试；S8/S9/S10 TODO 已标注依赖。 |
...
- Deliverables: `src/PieceTree.TextBuffer/Core` 新增 PieceTreeNode/PieceTreeModel/RB helpers，附 stub search API。
...
- [ ] `src/PieceTree.TextBuffer/Core` 含 PieceTreeNode、PieceTreeModel、RB helpers，`dotnet test` 日志附在 QA 产物或 meeting log 中。
- [ ] `PieceTree.TextBuffer.Tests` 内的 QA 矩阵与 baseline 日志存档，与 PT-005 运行记录互相引用。
...
- 2025-11-19：PT-004.G1 / PT-004.M2（skeleton wiring）—— `PieceTreeBuilder`/`PieceTreeBuffer` 走通 ChunkBuffer→PieceTreeModel，`dotnet test PieceTree.TextBuffer.Tests/PieceTree.TextBuffer.Tests.csproj`（Total 4, Passed 4），Porting Log 记录。
- 2025-11-19：PT-005.G1 —— `src/PieceTree.TextBuffer.Tests/TestMatrix.md` 建立，`UnitTest1.cs` 扩展至 7 个 Fact 覆盖 Plain/CRLF/Multi-chunk/metadata，并记录基线 `dotnet test`（Total 7, Passed 7）。
