...
| R19 | 2025-11-24 | Porter-CS | B3 DocUI Staged Fixes (FindDecorations reset + caret overlap) | `FindDecorations.Reset()` 保留 `_startPosition`，`IntervalTree.CollectOverlaps()` 捕获零宽查询；`DocUIFindModelTests` 新增 **Test48_FlushEditKeepsFindNextProgress**，`DocUIFindDecorationsTests` 增补 **CollapsedCaretAtMatchStartReturnsIndex**。QA rerun：`PIECETREE_DEBUG=0 dotnet test tests/TextBuffer.Tests/TextBuffer.Tests.csproj --filter FullyQualifiedName~FindModelTests --nologo` 46/46、`--filter FullyQualifiedName~DocUIFindDecorationsTests --nologo` 9/9。详见 `agent-team/handoffs/B3-DocUI-StagedFixes-20251124.md` / `agent-team/handoffs/B3-DocUI-StagedFixes-QA-20251124.md` / `docs/reports/migration-log.md`（B3-DocUI-StagedFixes 行） / [`agent-team/indexes/README.md#delta-2025-11-24-b3-docui-staged`](agent-team/indexes/README.md#delta-2025-11-24-b3-docui-staged)。 | 跟踪 B3-FM multi-selection（B3-FM-MSel-*）与 `FullyQualifiedName~DocUIFindModelTests` filter alias 清理，确保后续 CI/Doc 引用统一。 |
