...
| CL4 | Decorations、IntervalTree stickiness、Markdown DocUI rendering semantics（TS `textModelDecorations.ts`、`modelDecorations.ts`、`markdownRenderer.ts`） | `agent-team/handoffs/AA3-007-Audit.md` | `agent-team/handoffs/AA3-008-Result.md` | `DecorationTests`、`MarkdownRendererTests` | Audit Complete – Fixes Landed | [`AA3-007-Audit`](../../agent-team/handoffs/AA3-007-Audit.md)<br>[`AA3-008-Result`](../../agent-team/handoffs/AA3-008-Result.md) |
...
- **Validation Hooks:** `DecorationTests` 新增 metadata round-trip + event flag coverage；`MarkdownRendererTests` 覆盖 owner filter lists、injected text、glyph/minimap 注解；`dotnet test src/PieceTree.TextBuffer.Tests/PieceTree.TextBuffer.Tests.csproj`（85/85）。
