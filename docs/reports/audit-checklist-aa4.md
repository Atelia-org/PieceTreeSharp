...
| CL6 | ChangeBuffer/CRLF/large edits（TS `_insert/_delete`逻辑、`_lastChangeBufferPos`、AverageBufferSize） vs `PieceTreeModel.Edit.cs` | `agent-team/handoffs/AA4-002-Audit.md` | `agent-team/handoffs/AA4-006-Result.md` | `PieceTreeBaseTests` / 新增 ChangeBuffer fuzz & CRLF bridging cases | QA: Failing (CRLF repair - node boundary handling) — Needs Porter-CS follow-up | [`AA4-002-Audit`](../../agent-team/handoffs/AA4-002-Audit.md) |
