...
## Live Checkpoints (2025-11-22)
- **关键认知**：Investigator-TS 已完成首轮 TS 测试清单，并在 2025-11-22 新增 WordSeparator/SearchContext 细化说明：TS 的 `wordHelper.ts` + `WordCharacterClassifier.ts` + `TextModelSearch.ts` 形成共享缓存，C# 仍缺统一 `WordSeparatorMap`/`SearchContext`；DocUI find widget 测试集中于 `ts/src/vs/editor/contrib/find/test/browser`（`findModel.test.ts`、`findController.test.ts`、`find.test.ts`、`replacePattern.test.ts`），尚无 DOM snapshot 版本，需自建 harness。Porter-CS 已将 Batch #1 ReplacePattern API/控制器/测试清单固化（含 parser/case-preserver、`DocUIReplaceController`、JSON fixtures、Markdown snapshots）；QA 具备相应测试模板；Info-Indexer/DocMaintainer 已筹划 `#delta-2025-11-22` changefeed 与文档同步流程。
- **进度**：Appendix 表格新增 9 条记录 + 1 条 TODO；Investigator 记忆文件更新，锁定 CL8 (DocUI Find) 依赖和 fuzz/WordSeparator 阻塞；`src/PieceTree.TextBuffer.Tests/TestMatrix.md` 现包含 “TS Source” 与 “Portability Tier” 列，已为 PieceTree/TextModel/Diff/Decoration/Markdown 套件以及 DocUI placeholders 建档，并写入 Batch #1 QA 命令（`dotnet test` baseline + `DocUIReplacePatternTests`/`MarkdownRendererDocUI` filters）。Porter-CS 规划了 Batch #1 交付（`ReplacePattern.cs`, `DocUIReplaceController`, harness/fixtures、`DocUIReplacePatternTests.cs`），并定义文档/日志落点；Info-Indexer 完成 Batch #1 changefeed 工作流设计并细化从收集 Porter/QA artifact 到发布 `agent-team/indexes/README.md#delta-2025-11-22` 的操作流程；Planner 于 2025-11-22 协调简报中将 deliverables 映射到 Task Board（AA4-008/009/OI-011）并拟定 11/23–11/26 节点；QA-Automation 拆解 TS `replacePattern.test.ts` 并固化 fixtures/命令/snapshot 流程；DocMaintainer 列出了 Batch #1 文档接入点（AGENTS/Sprint/Task Board/Plan/迁移日志）与统一措辞，等待 changefeed 发布后执行。
- **进度**：Appendix 表格新增 9 条记录 + 1 条 TODO；Investigator 记忆文件更新，锁定 CL8 (DocUI Find) 依赖和 fuzz/WordSeparator 阻塞；`src/PieceTree.TextBuffer.Tests/TestMatrix.md` 现包含 “TS Source” 与 “Portability Tier” 列，已为 PieceTree/TextModel/Diff/Decoration/Markdown 套件以及 DocUI placeholders 建档，并写入 Batch #1 QA 命令（`dotnet test` baseline + `DocUIReplacePatternTests`/`MarkdownRendererDocUI` filters）。Porter-CS 已启动 Batch #1 实作：准备新增 `ReplacePattern.cs`、`DocUIReplaceController.cs` 与 `DocUIReplacePatternTests.cs`/fixtures/snapshots、在关键位置预置 TODO 标记，并列出后续需要 Investigator 提供 WordSeparator 规格、QA 输出 fixture JSON；Info-Indexer 完成 Batch #1 changefeed 工作流设计并细化从收集 Porter/QA artifact 到发布 `agent-team/indexes/README.md#delta-2025-11-22` 的操作流程；Planner 于 2025-11-22 协调简报中将 deliverables 映射到 Task Board（AA4-008/009/OI-011）并拟定 11/23–11/26 节点；QA-Automation 拆解 TS `replacePattern.test.ts` 并固化 fixtures/命令/snapshot 流程；DocMaintainer 列出了 Batch #1 文档接入点（AGENTS/Sprint/Task Board/Plan/迁移日志）与统一措辞，等待 changefeed 发布后执行。
- **后续思路**：进入实施阶段：Porter 按 checklist 先落 ReplacePattern runtime + DocUI harness + `DocUIReplacePatternTests`，并提供 fixture JSON 导出脚本、`dotnet test` 命令/commit hash 供 QA 复现；QA 待代码登陆后立即创建 `resources/docui/replace-pattern/`、`__snapshots__/docui/replace-pattern/` 等路径，按矩阵命令（含 `DOCUI_SNAPSHOT_RECORD=1`）生成 TRX + Markdown snapshot，并更新 `TestMatrix.md` 及 `agent-team/handoffs/AA4-009-QA.md`；Planner 按节点更新 Task Board/Sprint 文档，驱动 Porter→QA→Info-Indexer→DocMaintainer 接力；Info-Indexer 完成 `#delta-2025-11-22` 广播后，DocMaintainer 依照预案同步 AGENTS/Sprint/Task Board/Plan/迁移日志；并行准备 OI backlog（DocUI widget 测试路径、Snapshot tooling、WordSeparator parity、DocUI harness设计）为下一批 TS 测试迁移扫清阻碍。
