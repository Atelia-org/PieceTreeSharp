...
- **关键认知**：Investigator-TS 已完成首轮 TS 测试清单，并在 2025-11-22 新增 WordSeparator/SearchContext 细化说明：TS 的 `wordHelper.ts` + `WordCharacterClassifier.ts` + `TextModelSearch.ts` 形成共享缓存，C# 仍缺统一 `WordSeparatorMap`/`SearchContext`；DocUI find widget 测试集中于 `ts/src/vs/editor/contrib/find/test/browser`（`findModel.test.ts`、`findController.test.ts`、`find.test.ts`、`replacePattern.test.ts`），尚无 DOM snapshot 版本，需自建 harness。唯有 `replacePattern.test.ts` 属 Tier A，其余因上述依赖列为 Tier B/C。
- **进度**：Appendix 表格新增 9 条记录 + 1 条 TODO；Investigator 记忆文件更新，锁定 CL8 (DocUI Find) 依赖和 fuzz/WordSeparator 阻塞；`src/PieceTree.TextBuffer.Tests/TestMatrix.md` 现包含 “TS Source” 与 “Portability Tier” 列，已为 PieceTree/TextModel/Diff/Decoration/Markdown 套件以及 DocUI placeholders 建档，并写入 Batch #1 QA 命令（`dotnet test` baseline + `DocUIReplacePatternTests`/`MarkdownRendererDocUI` filters）。Porter-CS 规划了 Batch #1 交付（`ReplacePattern.cs`, `DocUIReplaceController`, harness/fixtures、`DocUIReplacePatternTests.cs`），并定义文档/日志落点（迁移日志 + changefeed + plan checkpoint）；Info-Indexer 完成 Batch #1 changefeed 工作流设计，指定 `agent-team/indexes/README.md#delta-2025-11-22` 为统一锚点；Planner 于 2025-11-22 协调简报中将 deliverables 映射到 Task Board（AA4-008/009/OI-011）并拟定 11/23–11/26 节点；QA-Automation 拆解 TS `replacePattern.test.ts` 并固化 fixtures/命令/snapshot 流程；DocMaintainer 列出了 Batch #1 文档接入点（AGENTS/Sprint/Task Board/Plan/迁移日志）与统一措辞，等待 changefeed 发布后执行。
- **后续思路**：QA 已完成矩阵扩展并锁定 ReplacePattern 具体测试/fixture/snapshot 流程，Porter Batch #1 计划就绪，下一步进入实现：先落 ReplacePattern runtime + tests，再搭建 DocUI harness；QA 完成代码合入后需按矩阵命令生成 TRX + Markdown snapshot 并更新 `TestMatrix.md`；Planner 将在 Task Board/Sprint 文档加入对应 checklist 与日期，驱动 Porter→QA→Info-Indexer→DocMaintainer 接力；Info-Indexer 在 Batch #1 合入后同步更新 AGENTS/Sprint/Task Board/migration log/TestMatrix 并执行验证清单；同时准备 OI backlog（DocUI widget 测试路径、Snapshot tooling、WordSeparator parity、DocUI harness设计）以支撑下一批 TS 测试迁移。
