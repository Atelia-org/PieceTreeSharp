...
- **2025-11-24:** QA validation for DocUI FindModel scope review (`#delta-2025-11-24-find-scope`) — Read `AGENTS.md`, `agent-team/members/qa-automation.md`, `agent-team/handoffs/AA4-Review-Porter.md`, `docs/reports/migration-log.md` (B3-FM-Scope), and `agent-team/indexes/README.md#delta-2025-11-24-find-scope` before reviewing staged diffs in `FindModel.cs`, `DocUIFindModelTests.cs`, and `TestMatrix.md`. Confirmed new pending-scope override plumbing plus Tests 45/46 guarded edits & normalization, and verified the TestMatrix targeted rerun row already cites the delta. Commands run: `PIECETREE_DEBUG=0 dotnet test ... --filter FullyQualifiedName~FindModelTests --nologo` (44/44) and `PIECETREE_DEBUG=0 dotnet test ... --filter FullyQualifiedName~DocUIFind --nologo` (39/39; broader set now includes DocUIFindDecorations). Residual risk: DocUIFind filter expectation in earlier QA docs still states 17/17—needs refresh to reflect the expanded DocUI suites prompting 39 hits.
- **2025-11-24:** QA-Automation DocUI staged fixes validation (B3) — Reviewed AI Playbook + QA memory per protocol, then executed: (1) `PIECETREE_DEBUG=0 dotnet test tests/TextBuffer.Tests/TextBuffer.Tests.csproj --filter FullyQualifiedName~DocUIFindModelTests --nologo` (0/0 due to alias mismatch), (2) `PIECETREE_DEBUG=0 dotnet test ... --filter FullyQualifiedName~FindModelTests --nologo` (46/46 green, covers `DocUIFindModelTests.Test48_FlushEditKeepsFindNextProgress`), and (3) `PIECETREE_DEBUG=0 dotnet test ... --filter FullyQualifiedName~DocUIFindDecorationsTests --nologo` (9/9 green, includes `CollapsedCaretAtMatchStartReturnsIndex`). Updated `tests/TextBuffer.Tests/TestMatrix.md` (DocUI rows + targeted reruns), logged results in `agent-team/handoffs/B3-DocUI-StagedFixes-QA-20251124.md`, and captured filter alias drift + remaining multi-cursor TODOs as risks. Next: ensure future CI scripts use the correct `FindModelTests` identifier or rename the suite to match DocUI docs.
