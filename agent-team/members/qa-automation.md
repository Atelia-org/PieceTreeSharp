...
- **2025-11-21:** AA4-009 revalidation after Porter-CS drop — Ran `export PIECETREE_DEBUG=0 && dotnet test tests/TextBuffer.Tests/TextBuffer.Tests.csproj --nologo` (119/119, 7.4s), builder/factory spot check `export PIECETREE_DEBUG=0 && dotnet test ... --filter "FullyQualifiedName~PieceTreeBuilderTests|FullyQualifiedName~PieceTreeFactoryTests" --nologo` (7/7, 2.2s), and deterministic fuzz harness `export PIECETREE_DEBUG=0 PIECETREE_FUZZ_LOG_DIR=/tmp/aa4-009-fuzz-logs dotnet test ... --filter FullyQualifiedName~CRLF_RandomFuzz_1000 --nologo` (1/1, 2.9s, seed 123). Logged results in `tests/TextBuffer.Tests/TestMatrix.md`, refreshed `agent-team/handoffs/AA4-009-QA.md`, and documented `/tmp/aa4-009-fuzz-logs` as the standing location for redirected fuzz logs (none emitted due to all tests passing).
...
- `export PIECETREE_DEBUG=0 PIECETREE_FUZZ_LOG_DIR=/tmp/docui-fuzz dotnet test tests/TextBuffer.Tests/TextBuffer.Tests.csproj --nologo --filter FullyQualifiedName~DocUIReplaceCaptureFuzzTests --settings tests/RunSettings/Fuzz.runsettings` (F3 fuzz/perf smoke with logged seeds + runtime).
...
- **2025-11-23:** QA-Automation DocUI Find validation (Batch #3 delta-2025-11-23-docuifind) — Reviewed staged diffs for `DocUIFindController.cs`, `FindModel.cs`, `FindUtilities.cs`, `DocUI` test suites; confirmed `DocUIFindModelTests.Test28/29` + selection seed helpers landed; noted missing staged coverage for preserve-case toggle and empty-clipboard guard. Ran `export PIECETREE_DEBUG=0 && dotnet test tests/TextBuffer.Tests/TextBuffer.Tests.csproj --filter FullyQualifiedName~DocUIFind --nologo` (17/17 green, 1.9s — historical count before DocUIFindDecorations expanded the filter to 39 tests on 2025-11-24) and logged the command in `tests/TextBuffer.Tests/TestMatrix.md`. Captured QA summary + coverage gaps in `agent-team/handoffs/AA4-009-QA.md`.
 - **2025-11-23:** DocUI Find restage check — Reopened staging diff after Porter restage to ensure preserve-case toggle default, host word separator plumbing, and empty clipboard guard are present. Re-ran `export PIECETREE_DEBUG=0 && dotnet test tests/TextBuffer.Tests/TextBuffer.Tests.csproj --filter FullyQualifiedName~DocUIFind --nologo` (17/17 green, 1.6s — same historical count; now 39/39 post-Decorations drop) to refresh the QA log, updated `tests/TextBuffer.Tests/TestMatrix.md` targeted reruns row, and appended the addendum + TODOs to `agent-team/handoffs/AA4-009-QA.md`.
- **2025-11-23:** QA-Automation DocUI FindController verification — Synced staged diffs for `src/TextBuffer/DocUI/DocUIFindController.cs`, `tests/TextBuffer.Tests/DocUI/DocUIFindControllerTests.cs`, and `tests/TextBuffer.Tests/TestMatrix.md`; confirmed auto-scope fallback, previous-command loops, select-all updates, and clipboard/preserve-case toggles align with Investigator findings. Executed `dotnet test tests/TextBuffer.Tests/TextBuffer.Tests.csproj --filter DocUIFindControllerTests --nologo` (20/20 green, 1.8s) to validate deterministic behavior; documented helper coverage gap for `StartFindWithSelectionAction` (no direct test yet) for follow-up in AA4-009 QA notes.
- **2025-11-23:** QA-Automation B3-FC lifecycle verification (`#delta-2025-11-23-b3-fc-lifecycle`) — Reviewed staged diffs for `DocUIFindController.cs`, `DocUIFindControllerTests.cs`, `DocUIFindSelectionTests.cs`, `TestMatrix.md`, `docs/plans/ts-test-alignment.md`, and `docs/reports/migration-log.md` to confirm Ctrl+F reseed parity, SeedSearchStringMode.Never behavior, find-model disposal, Cmd+E coverage, and replace panel reset notes landed. Observed documentation gaps: TestMatrix still lists **Total Tests Passing: 204** while current full run is 217, and `#delta-2025-11-23-b3-fc-lifecycle` is referenced in TestMatrix/plan/migration docs but is missing from `agent-team/indexes/README.md`. Executed targeted + full suites to validate fixes: `dotnet test tests/TextBuffer.Tests/TextBuffer.Tests.csproj --filter DocUIFindControllerTests --nologo` (26/26 green, 1.7s), `dotnet test tests/TextBuffer.Tests/TextBuffer.Tests.csproj --filter DocUIFindSelectionTests --nologo` (4/4 green, 1.6s), and `dotnet test tests/TextBuffer.Tests/TextBuffer.Tests.csproj --nologo` full sweep (217/217 green, 2.7s). Residual risk: changefeed entry still pending, so downstream consumers cannot yet trace lifecycle delta provenance; flagged in QA notes for follow-up.
...
- **2025-11-24:** QA validation for DocUI FindModel scope review (`#delta-2025-11-24-find-scope`) — Read `AGENTS.md`, `agent-team/members/qa-automation.md`, `agent-team/handoffs/AA4-Review-Porter.md`, `docs/reports/migration-log.md` (B3-FM-Scope), and `agent-team/indexes/README.md#delta-2025-11-24-find-scope` before reviewing staged diffs in `FindModel.cs`, `DocUIFindModelTests.cs`, and `TestMatrix.md`. Confirmed new pending-scope override plumbing plus Tests 45/46 guarded edits & normalization, and verified the TestMatrix targeted rerun row already cites the delta. Commands run: `export PIECETREE_DEBUG=0 && dotnet test ... --filter FullyQualifiedName~FindModelTests --nologo` (44/44) and `export PIECETREE_DEBUG=0 && dotnet test ... --filter FullyQualifiedName~DocUIFind --nologo` (39/39; broader set now includes DocUIFindDecorations). Residual risk: DocUIFind filter expectation in earlier QA docs still states 17/17—needs refresh to reflect the expanded DocUI suites prompting 39 hits.
- **2025-11-24:** QA-Automation DocUI staged fixes validation (B3) — Reviewed AI Playbook + QA memory per protocol, then executed: (1) `export PIECETREE_DEBUG=0 && dotnet test tests/TextBuffer.Tests/TextBuffer.Tests.csproj --filter FullyQualifiedName~FindModelTests --nologo` (46/46 green, covers `DocUIFindModelTests.Test48_FlushEditKeepsFindNextProgress`), and (2) `export PIECETREE_DEBUG=0 && dotnet test ... --filter FullyQualifiedName~DocUIFindDecorationsTests --nologo` (9/9 green, includes `CollapsedCaretAtMatchStartReturnsIndex`). Updated `tests/TextBuffer.Tests/TestMatrix.md` (DocUI rows + targeted reruns), logged results in `agent-team/handoffs/B3-DocUI-StagedFixes-QA-20251124.md`, and explicitly documented that the DocUI alias has been retired to avoid 0/0 reruns. Next: ensure future CI scripts use the correct `FindModelTests` identifier or rename the suite to match DocUI docs.
- **2025-11-24:** B3-FM multi-selection QA close-out — Executed targeted commands with canonical filters (`Test07/08` pair 2/2 @1.7s, `FullyQualifiedName~FindModelTests` 48/48 @3.3s, full `dotnet test` 242/242 @2.9s) using `export PIECETREE_DEBUG=0` to validate the updated harness + `FindModel.SetSelections`. Created `agent-team/handoffs/B3-FM-MultiSelection-QA.md`, refreshed `tests/TextBuffer.Tests/TestMatrix.md` DocUI FindModel row (now 43/43 coverage with command list), and noted remaining alias drift risk for Info-Indexer. Ready for DocMaintainer/Info-Indexer to publish `#delta-2025-11-24-b3-fm-multisel`.
- **2025-11-24:** DocUI FindModel QA rerun (`#delta-2025-11-24-find-primary`) — Reloaded QA memory per protocol, then executed `export PIECETREE_DEBUG=0 && dotnet test tests/TextBuffer.Tests/TextBuffer.Tests.csproj --filter "FullyQualifiedName~FindModelTests" --nologo` (49/49 green, 2.6s) to capture Tests07/08/49 evidence. Updated `tests/TextBuffer.Tests/TestMatrix.md` DocUI FindModel row + targeted rerun entry with the new baseline and confirmed no regressions; no follow-ups required beyond monitoring changefeed consumers.
- **2025-11-24:** B3 PieceTree Fuzz Review QA — Reviewed Investigator/Porter handoffs (CI-2/W-1/W-2 scope), confirmed staged diffs for `PieceTreeFuzzHarness.cs`, `PieceTreeModel.cs`, `PieceTreeModel.Search.cs`, `PieceTreeBuffer.cs`, `PieceTreeFuzzHarnessTests.cs`, and `FuzzLogCollector.cs`, then executed `export PIECETREE_DEBUG=0 && dotnet test tests/TextBuffer.Tests/TextBuffer.Tests.csproj --filter PieceTreeFuzzHarnessTests --nologo --results-directory tests/TextBuffer.Tests/TestResults --logger "trx;LogFileName=b3-piecetree-fuzz-harness.trx"` (2/2 green) and `export PIECETREE_DEBUG=0 && dotnet test tests/TextBuffer.Tests/TextBuffer.Tests.csproj --filter "FullyQualifiedName~PieceTreeModelTests" --nologo --results-directory tests/TextBuffer.Tests/TestResults --logger "trx;LogFileName=b3-piecetree-model.trx"` (9/9 green). Logged outcomes in `agent-team/handoffs/B3-PieceTree-Fuzz-Review-QA.md` with TRX paths for reproducibility and noted CI-1/DOC-1 remain outstanding.
- **2025-11-24:** B3 sentinel/getLineContent QA verification — Re-read AGENTS + QA memory, then executed the mandated suite for `#delta-2025-11-24-b3-sentinel` / `#delta-2025-11-24-b3-getlinecontent`: (1) `export PIECETREE_DEBUG=0 && dotnet test -v m` (253/253/0, 59.3s) for full coverage, (2) cache invalidation filter (2/2/0, 3.8s), (3) normalization filter (3/3/0, 1.6s), and (4) `PieceTreeFuzzHarnessTests.RandomDeleteThreeMatchesTsScript` (1/1/0, 1.6s). Updated `tests/TextBuffer.Tests/TestMatrix.md` baseline + targeted rerun tables with the new totals/durations, created `agent-team/handoffs/B3-TestFailures-QA.md`, and captured findings/conclusions for Info-Indexer.
