...
- **2025-11-22:** Batch #1 ReplacePattern QA prep — Audited workspace and confirmed DocUI replace-pattern scaffolding folders (`src/PieceTree.TextBuffer.Tests/resources/docui/replace-pattern/`, `src/PieceTree.TextBuffer.Tests/__snapshots__/docui/replace-pattern/`, `src/PieceTree.TextBuffer.Tests/DocUI/`) plus `TestResults/` do not exist yet; outlined creation order tied to Porter’s Batch #1 drop. Documented prerequisites (TS fixture export → `cases.json`, snapshot regeneration env `DOCUI_SNAPSHOT_RECORD=1`, TRX capture paths) and the need for Porter to post commit hash + `dotnet test` filter/seed command bundle before QA baselines and filtered reruns (`DocUIReplacePatternTests` + Batch #1 full sweep) can proceed.
