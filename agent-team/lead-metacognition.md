...
- **增强保留原则** (2025-12-02)：当 C# 实现与 TS 存在差异时，如果已有实现优于原版，就尽量保留（"增强"而非"偏差"）
- **快速胜利优先** (2025-12-02)：优先处理低复杂度高产出的任务，积累动量后再攻克复杂任务
...
### DocUI & LLM-Native IDE 愿景 (2025-12-02 更新)

**近期目标** (Sprint 04 后)：
- 探索与 DocUI 项目对接
- 开发面向 LLM 的文本编辑器组件
- 原型名称：`LiveContextProto`

**长期目标**：
- 开发 **LLM-Native & Friendly 的编程 IDE**
- LLM 是这个 IDE 的**用户和使用者**（不是助手，是主体）
- 功能通过 DocUI 渲染到 LLM 上下文中
- 根据 LLM 特性设计交互流程
- 深度整合 Roslyn (C# 语义分析)
