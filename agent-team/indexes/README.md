...
- **QA evidence:** `export PIECETREE_DEBUG=0 && dotnet test tests/TextBuffer.Tests/TextBuffer.Tests.csproj --filter FullyQualifiedName~TextModelSearchTests --nologo` → 45/45 green (2.0s, R36) along with the single-case Issue #53415 regression (`FullyQualifiedName=...Issue53415...`) noted in the memo; results logged in `tests/TextBuffer.Tests/TestMatrix.md` under the same delta.
- **Documentation hooks:** `docs/plans/ts-test-alignment.md` Appendix now marks the TextModelSearch gap as closed and cites both handoffs; `docs/reports/migration-log.md` row **B3-TextModelSearch** and `docs/sprints/sprint-03.md` (R35/R36) describe the investigation + helper/test import; `tests/TextBuffer.Tests/TestMatrix.md` embeds the rerun command and backlog reminder (Intl.Segmenter + WordSeparator cache perf), and all Task Board/AGENTS references should use this changefeed when mentioning TextModelSearch parity.
