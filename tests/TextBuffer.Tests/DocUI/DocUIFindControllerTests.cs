...
public sealed class DocUIFindControllerTests
{
    ...
    {
        TestEditorHost host = new(["ABC", "ABC", "XYZ", "ABC"]);
        using DocUIFindController controller = CreateController(host);
        FindReplaceState state = controller.State;
    ...
    {
        FindControllerHostOptions options = new()
        {
            SeedSearchStringFromSelection = SeedSearchStringMode.Always
        };
        TestEditorHost host = new(["import nls = require('vs/nls');"], options);
        using DocUIFindController controller = CreateController(host);
    ...
    public void CtrlFReseedsFromSelectionEvenWhenSearchStringExists()
    {
        ...
        TestEditorHost host = new(["foo bar"], options);
        using DocUIFindController controller = CreateController(host);
    ...
    public void StartFindReplaceHonorsNeverSeedPreference()
    {
        ...
        TestEditorHost host = new(["foo", "bar"], options);
        TestFindControllerClipboard clipboard = new(isEnabled: true, initial: "clip");

        using DocUIFindController controller = CreateController(host, clipboard: clipboard);
    ...
    public void Issue9043_SearchScopeClearsWhenHidden()
    {
        ...
        Range scopedRange = new(new TextPosition(1, 1), new TextPosition(1, 5));
        controller.State.Change(searchScope: [scopedRange], searchScopeProvided: true, moveCursor: false);
    ...
    public void Issue27083_SearchScopeUpdatesWhenVisible()
    {
        ...
        using DocUIFindController controller = CreateController(host);
        FindStartOptions startOptions = new()
        {
            ForceRevealReplace = false,
            SeedSearchStringFromSelection = SelectionSeedMode.None,
            SeedSearchStringFromNonEmptySelection = false,
    ...
    public void SearchScopePersistsWhenSelectionCollapses()
    {
        ...
        controller.Start(startOptions);
        Range scopedRange = new(new TextPosition(1, 5), new TextPosition(1, 11));
        AssertScopes(controller, scopedRange);
    ...
    {
        TestEditorHost host = new(["foo bar baz"]);
        using DocUIFindController controller = CreateController(host);
    ...
    public void SelectAllMatchesActionAppliesSelections()
    {
        ...
        Selection[] selections = host.GetSelections();
    ...
    public void DefaultPreserveCaseOptionHydratesState()
    {
        ...
        TestEditorHost host = new(["text"], options);
    ...
    {
        TestFindControllerStorage storage = new();

        TestEditorHost hostA = new(["alpha"]);
        using (DocUIFindController controller = CreateController(hostA, storage))
        {
        ...
        TestEditorHost hostB = new(["beta"]);
        using DocUIFindController controllerB = CreateController(hostB, storage);
    ...
    public void EmptyGlobalClipboardDoesNotClearSearchString()
    {
        ...
        TestEditorHost host = new(["foo"], options);
        TestFindControllerClipboard clipboard = new(isEnabled: true, initial: string.Empty);
    ...
    {
        Selection selection = host.GetSelections().Single();
    ...
    {
        Range[]? scope = controller.State.SearchScope;
    ...
    private sealed class TestEditorHost : IEditorHost
    {
        ...
        public TestEditorHost(string[] lines, FindControllerHostOptions? options = null)
        {
            ...
            _selections = [new Selection(TextPosition.Origin, TextPosition.Origin)];
        }
        ...
        {
            TextEdit[] array = edits?.ToArray() ?? Array.Empty<TextEdit>();
            if (array.Length == 0)
            {
                return;
            }
        ...
        {
            TextPosition end = _model.GetPositionAt(_model.GetLength());
            _model.ApplyEdits([new TextEdit(TextPosition.Origin, end, value ?? string.Empty)]);
        }
    ...
    private sealed class TestFindControllerStorage : IFindControllerStorage
    {
        ...
        {
            return _values.TryGetValue(key, out bool value) ? value : null;
        }
