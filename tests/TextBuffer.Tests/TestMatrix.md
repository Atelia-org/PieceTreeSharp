...
| TextModelSearchTests | Regex/word search parameters, CRLF payloads, `parseSearchRequest` + navigation parity | [ts/src/vs/editor/test/common/model/textModelSearch.test.ts](../../ts/src/vs/editor/test/common/model/textModelSearch.test.ts) | B | Verified (Sprint 03 R36 – parseSearchRequest + navigation) | `#delta-2025-11-25-b3-textmodelsearch`: Investigator brief [`Review-20251125-Investigator.md`](../../agent-team/handoffs/Review-20251125-Investigator.md) + Porter memo [`B3-TextModelSearch-PORT.md`](../../agent-team/handoffs/B3-TextModelSearch-PORT.md) confirm `SearchPatternUtilities.IsMultilineRegexSource` landed and the TS 45-case battery now lives here. Latest QA command (2025-11-26 Investigator rerun) `export PIECETREE_DEBUG=0 && dotnet test tests/TextBuffer.Tests/TextBuffer.Tests.csproj --filter FullyQualifiedName~TextModelSearchTests --nologo` → 45/45 green (2.8s); DocMaintainer cross-checked `agent-team/handoffs/DocReview-20251126-INV.md` + `agent-team/handoffs/DocFix-20251126-Porter.md` so downstream docs cite the same evidence. Backlog: Intl.Segmenter word segmentation + WordSeparator cache perf remain open. |
...
| `export PIECETREE_DEBUG=0 && dotnet test tests/TextBuffer.Tests/TextBuffer.Tests.csproj --filter TextModelSearchTests --nologo` | 45/45 green (2.8s) | Replays the entire TextModelSearch battery after the new `parseSearchRequest` + navigation suites landed; Investigator rerun evidence (2025-11-26) logged in `agent-team/handoffs/DocReview-20251126-INV.md` for changefeed `#delta-2025-11-25-b3-textmodelsearch`. |
