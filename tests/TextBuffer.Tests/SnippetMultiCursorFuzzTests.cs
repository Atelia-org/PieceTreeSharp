    ...
    public class SnippetMultiCursorFuzzTests
    {
        ...
        public void SnippetAndMultiCursor_Fuzz_NoCrashesAndInvariantsHold()
        {
            ...
            for (int i = 0; i < Iterations; i++)
            {
                ...
                var baseText = "base-" + Guid.NewGuid().ToString("N").Substring(0, 12);
                var model = new TextModel(baseText);
                var expectedLen = model.GetLength();

                // Create cursor collection and random cursors
                var collection = model.CreateCursorCollection();
                var cursorCount = rng.Next(1, 4);
                var positions = new TextPosition[cursorCount];
                for (int c = 0; c < cursorCount; c++)
                {
                    var offset = rng.Next(0, model.GetLength() + 1);
                    positions[c] = model.GetPositionAt(offset);
                    var cursor = collection.CreateCursor(positions[c]);
                }

                // Create a single snippet session and insert multiple snippets across cursors
                var controller = model.CreateSnippetController();
                ...
                {
                    var pos = positions[c];
                    var token = $"PLH_{i}_{c}";
                    var snippet = "${1:" + token + "}"; // ${1:PLH_i_c}
                ...
                var placeholders = decorations.Where(d => d.Options.Description == "snippet-placeholder").ToArray();
                ...
                while ((p = controller.NextPlaceholder()) != null)
                {
                    navCount++;
                    var pos = p.Value;
                    var startOff = model.GetOffsetAt(pos);

                    // Find the corresponding decoration with the same start offset
                    var dec = placeholders.FirstOrDefault(d => d.Range.StartOffset == startOff);
                    Assert.NotNull(dec);

                    var decRange = dec.Range;
                    var decStartPos = model.GetPositionAt(decRange.StartOffset);
                    var decEndPos = model.GetPositionAt(decRange.EndOffset);

                    // Replace placeholder content with a new small string
                    var newContent = "Z" + rng.Next(0, 99).ToString();
                    model.PushEditOperations(new[] { new TextEdit(decStartPos, decEndPos, newContent) });
                ...
                var cursorPositions = collection.GetCursorPositions();
                foreach (var cp in cursorPositions)
                {
                    var off = model.GetOffsetAt(cp);
