...
internal sealed class DynamicProgrammingDiffing : IDiffAlgorithm
{
    public DiffAlgorithmResult Compute(ISequence sequence1, ISequence sequence2, ITimeout? timeout = null, Func<int, int, double>? equalityScore = null)
    {
        ...
        Array2D<double> lcsLengths = new(sequence1.Length, sequence2.Length);
        Array2D<int> directions = new(sequence1.Length, sequence2.Length);
        Array2D<int> lengths = new(sequence1.Length, sequence2.Length);
        ...
        for (int s1 = 0; s1 < sequence1.Length; s1++)
        {
            for (int s2 = 0; s2 < sequence2.Length; s2++)
            {
                ...
                double horizontalLen = s1 == 0 ? 0 : lcsLengths.Get(s1 - 1, s2);
                double verticalLen = s2 == 0 ? 0 : lcsLengths.Get(s1, s2 - 1);
                ...
                if (direction == 3)
                {
                    int prevLen = (s1 > 0 && s2 > 0) ? lengths.Get(s1 - 1, s2 - 1) : 0;
        ...
        int lastAligningPosS1 = sequence1.Length;
        int lastAligningPosS2 = sequence2.Length;
        ...
        int i1 = sequence1.Length - 1;
        int i2 = sequence2.Length - 1;
        while (i1 >= 0 && i2 >= 0)
        {
            int dir = directions.Get(i1, i2);
            if (dir == 3)
            {
                Report(i1, i2);
                i1--;
