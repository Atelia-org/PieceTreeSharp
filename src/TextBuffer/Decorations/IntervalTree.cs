...
internal sealed class IntervalTree
{
    ...
    public void Insert(ModelDecoration decoration)
    {
        ...
        IntervalNode node = new(decoration.Id, range.StartOffset, range.EndOffset)
        {
    ...
    {
        if (_nodesById.TryGetValue(id, out IntervalNode? node))
        {
    ...
    {
        if (_nodesById.TryGetValue(decoration.Id, out IntervalNode? existing))
        {
    ...
    public IEnumerable<ModelDecoration> EnumerateFrom(int startOffset, int ownerFilter = DecorationOwnerIds.Any)
    {
        ...
        NormalizeDeltaIfNeeded();
        List<ModelDecoration> result = [];
        IntervalNode node = FindFirstNodeStartingAtOrAfter(startOffset);
        while (node != Sentinel)
        {
    ...
    public IEnumerable<ModelDecoration> EnumerateAll()
    {
        ...
        NormalizeDeltaIfNeeded();
        List<ModelDecoration> result = [];
        IntervalNode node = Minimum(_root);
    ...
    {
        TrackedRangeStickiness nodeStickiness = node.GetStickiness();
        ...
        {
            MarkerMoveSemantics moveSemantics = forceMoveMarkers ? MarkerMoveSemantics.ForceMove : (deletingCnt > 0 ? MarkerMoveSemantics.ForceStay : MarkerMoveSemantics.MarkerDefined);
        ...
        {
            MarkerMoveSemantics moveSemantics = deletingCnt > insertingCnt ? MarkerMoveSemantics.ForceStay : MarkerMoveSemantics.MarkerDefined;
        ...
        {
            MarkerMoveSemantics moveSemantics = forceMoveMarkers ? MarkerMoveSemantics.ForceMove : MarkerMoveSemantics.MarkerDefined;
    ...
    private static int ComputeMaxEnd(IntervalNode node)
    {
        ...
        if (node.Left != Sentinel)
        {
            int leftMaxEnd = node.Left.MaxEnd;
        ...
        {
            int rightMaxEnd = node.Right.MaxEnd + node.Delta;
    ...
    private IntervalNode Successor(IntervalNode node)
    {
        ...
        IntervalNode current = node;
        IntervalNode parent = current.Parent;
    ...
    {
        IntervalNode current = _root;
        IntervalNode candidate = Sentinel;
        int delta = 0;

        while (current != Sentinel)
        {
            int absoluteStart = current.Start + delta;
