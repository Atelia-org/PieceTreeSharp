...
namespace PieceTree.TextBuffer.Cursor
{
    ...
    public static class WordOperations
    {
        ...
        {
            var classifier = new WordCharacterClassifier(wordSeparators);
            var content = model.GetLineContent(from.LineNumber);
            var index = Math.Clamp(from.Column - 1, 0, content.Length);
        ...
        public static TextPosition MoveWordLeft(TextModel model, TextPosition from, string? wordSeparators)
        {
            ...
            if (index == 0)
            {
                ...
                {
                    var prevLine = model.GetLineContent(from.LineNumber - 1);
        ...
        {
            var newActive = MoveWordRight(model, current.Active, wordSeparators);
        ...
        {
            var newActive = MoveWordLeft(model, current.Active, wordSeparators);
            return new Selection(current.Anchor, newActive);
        }
